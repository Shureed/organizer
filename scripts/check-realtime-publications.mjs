 ...
 *
 * Env:
 *   SUPABASE_URL          — Cortex project URL (required unless --dry)
 *   SUPABASE_SECRET_KEY   — sb_secret_* key    (required unless --dry)
 *
 * Flags:
...
// ── 2. Fetch the supabase_realtime publication set via public.run_queries ───

async function fetchPublishedTables(url, key) {
  const endpoint = `${url.replace(/\/$/, '')}/rest/v1/rpc/run_queries`
  ...
  if (!res.ok) {
    ...
    throw new Error(`run_queries failed: ${res.status} ${res.statusText} — ${body}`)
  }

  const data = await res.json()
  // public.run_queries returns an array-of-arrays (one sub-array per query).
  // Normalize: accept either shape to be defensive.
  let rows = data
  if (Array.isArray(data) && Array.isArray(data[0])) rows = data[0]
  if (data && Array.isArray(data.results) && Array.isArray(data.results[0])) rows = data.results[0]

  if (!Array.isArray(rows)) {
    throw new Error(`run_queries returned unexpected shape: ${JSON.stringify(data).slice(0, 400)}`)
  }

  return rows.map((r) => ({
    schema: r.schemaname ?? r.schema,
...
async function main() {
  ...
  if (!url || !key) {
    console.error(
      '[realtime-pub-scan] ERROR: SUPABASE_URL and SUPABASE_SECRET_KEY must be set ' +
        '(or pass --dry / set SKIP_REALTIME_PUB_CHECK=1 to skip the remote fetch)',
    )
    return 1
  }
  ...
  for (const s of subs) {
    ...
    if (!publishedSet.has(key)) missing.push(s)
  }

  if (missing.length > 0) {
    console.error('[realtime-pub-scan] FAIL — the following subscribed tables are NOT in the supabase_realtime publication:')
    for (const m of missing) {
      console.error(`  - ${m.schema}.${m.table}  (${m.loc})`)
    }
    console.error('')
    console.error('  Remediation: add each missing table to the supabase_realtime publication in a migration, e.g.')
